...
const Credits = async () => {
  ...
  return (
    ...
    >
      ...
      {/* Name */}
      <div>
        <p className={`font-bold text-base ${isFeatured ? 'text-gray-900' : 'text-white'}`}>
          {plan.name}
        </p>
      ...
      <div className="flex items-baseline gap-1">
        <span className={`text-4xl font-bold ${isFeatured ? 'text-gray-900' : 'text-white'}`}>
          ${plan.price}
        </span>
          ...
          <li key={plan.name + inclusion.label}
            className={`flex items-center gap-2.5 text-sm
              ${inclusion.isIncluded
                ? isFeatured ? 'text-gray-700' : 'text-white/90'
                : isFeatured ? 'text-gray-300' : 'text-white/25'}
            `}
          >
            <Image
              ...
              width={16}
              height={16}
              className={!inclusion.isIncluded ? 'opacity-30' : ''}
            />
            {inclusion.label}
          </li>
        ))}
